...
（1）大多数守护进程通过syslog(3)函数产生日志消息，消息会被发送到**UNIX域数据报套接字** /dev/log 上。

（2）任何用户进程可以将日志消息发送到**网络套接字**udp端口514。
...
？accpet后 epoll_ctl常出现 no such file 问题。
